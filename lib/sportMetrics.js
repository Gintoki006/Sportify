...
export const SPORT_METRICS = {
  FOOTBALL: [
    ...
    { key: 'fouls', label: 'Fouls', type: 'int', defaultValue: 0 },
    {
      key: 'yellow_cards',
      label: 'Yellow Cards',
      type: 'int',
    ...
    { key: 'red_cards', label: 'Red Cards', type: 'int', defaultValue: 0 },
    {
      key: 'clean_sheets',
      label: 'Clean Sheets',
      type: 'int',
    ...
    { key: 'offsides', label: 'Offsides', type: 'int', defaultValue: 0 },
    {
      key: 'match_result',
...
/**
 * Team sports — tournaments use team names (e.g. "Mumbai XI"),
 * player linking happens at scoring time, not tournament creation.
 */
export const TEAM_SPORTS = ['FOOTBALL', 'CRICKET', 'BASKETBALL', 'VOLLEYBALL'];

/**
 * Individual sports — tournaments link members directly to bracket slots
 * at tournament creation time (1 player = 1 slot).
 */
export const INDIVIDUAL_SPORTS = ['TENNIS', 'BADMINTON'];

/**
 * Check whether a sport is a team sport.
 * @param {string} sportType — one of the SportType enum values
 * @returns {boolean}
 */
export function isTeamSport(sportType) {
  return TEAM_SPORTS.includes(sportType);
}
